...
public class Fish : MonoBehaviour
{
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (DataManager.Instance.DolppuPay == false)
        {
            ...
            {
                DataManager.Instance.PoketMoneyCurrent -= DataManager.Instance.FishCost;
                DataManager.Instance.FishCount += 1;
                if (DataManager.Instance.PoketMoneyCurrent >= 0)
                {
                    DataManager.Instance.Score += DataManager.Instance.FishScore;
                }
                else
                {
                    //보통엔딩인 계산대로 보내버리기
