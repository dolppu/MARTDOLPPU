...
public class Microwave : MonoBehaviour
{
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (DataManager.Instance.DolppuPay == false)
        {
            ...
            {
                DataManager.Instance.PoketMoneyCurrent -= DataManager.Instance.MicrowaveCost;
                DataManager.Instance.MicrowaveCount += 1;
                if (DataManager.Instance.PoketMoneyCurrent >= 0)
                {
                    DataManager.Instance.Score += DataManager.Instance.MicrowaveScore;
                }
                else
                {
                    //보통엔딩인 계산대로 보내버리기
