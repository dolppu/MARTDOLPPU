...
public class Plush : MonoBehaviour
{
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (DataManager.Instance.DolppuPay == false)
        {
            ...
            {
                DataManager.Instance.PoketMoneyCurrent -= DataManager.Instance.PlushCost;
                DataManager.Instance.PlushCount += 1;
                if (DataManager.Instance.PoketMoneyCurrent >= 0)
                {
                    DataManager.Instance.Score += DataManager.Instance.PlushScore;
                }
                else
                {
                    //보통엔딩인 계산대로 보내버리기
