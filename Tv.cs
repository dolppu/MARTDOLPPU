...
public class Tv : MonoBehaviour
{
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (DataManager.Instance.DolppuPay == false)
        {
            ...
            {
                DataManager.Instance.PoketMoneyCurrent -= DataManager.Instance.TvCost;
                DataManager.Instance.TvCount += 1;
                if (DataManager.Instance.PoketMoneyCurrent >= 0)
                {
                    DataManager.Instance.Score += DataManager.Instance.TvScore;
                }
                else
                {
                    //보통엔딩인 계산대로 보내버리기
